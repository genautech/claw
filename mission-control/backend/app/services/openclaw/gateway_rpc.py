...
async def openclaw_call(
    ...
    *,
    ...
    # #region debug log
    _debug_path = __import__("os").environ.get("DEBUG_LOG_PATH")
    if _debug_path:
        try:
            _tl = len(config.token) if config.token else 0
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "mc_backend",
                            "hypothesisId": "H2",
                            "location": "gateway_rpc.py:openclaw_call",
                            "message": "MC backend building gateway URL",
                            "data": {"hasToken": bool(config.token), "tokenLength": _tl},
    ...
    try:
        ...
        async with websockets.connect(gateway_url, ping_interval=None, additional_headers=custom_headers) as ws:
            first_message = None
            try:
